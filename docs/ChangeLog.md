...
### 2026-06-24 · 日志模块统一 + 颜色分级
- 新增 log.py 统一日志模块，tag 改为 [bili2note]
- info（青色）、warn（黄色）、error（红色）、progress（行内刷新）颜色分级
- 笔记路径输出改为 file:// 绝对路径，终端可点击打开
- run.sh 快启动脚本：自动创建 venv、安装依赖、运行

### 2026-06-24 · 音频下载双方案：playurl API 主力 + yt-dlp 降级
- 音频下载新增 playurl API 直链方案（主力）：通过 B站官方接口获取音频 CDN 地址，httpx 流式下载，不被 412 拦截
- yt-dlp 降级为备选，仅在 API 方案不可用时触发
- 下载进度改为百分比显示（与模型下载风格统一）
